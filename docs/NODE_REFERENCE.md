...
| **MiniMax H3 Contex Loop Plan** | Widgets: Scene Plan, `run_name`, size, duration, seed. Optional sockets: `chain_policy`, `project_assets`. | `plan`, `summary`, scene count, width, height, blend frames | Stores the production plan and generation identity. Use the visual editor instead of raw JSON for normal work. |
